...
const challenges = [
  {
    ...
    slug: 'pyhton',
    url:
      'https://github.com/Asabeneh/30-Days-Of-Python.com',
    author: {
      firstName: 'Asabeneh',
      lastName: 'Yetayeh',
    },
...
const NotFound = (props) => <h1>The page you are looking for not found</h1>
    ...
    <div>
      {isLoggedIn ? (
        <>
          ...
          <small>Now, you can navigate through all the challenges</small>
          <br />
        </>
      ) : (
        <p>Please login to access the challenges</p>
      )}
...
const Welcome = ({ isLoggedIn, handleLogin }) => {
  return (
    <div>
      {isLoggedIn ? 'Welcome to the challenge' : <p>Please login</p>}
      <button onClick={handleLogin}>{isLoggedIn ? 'Logout' : 'Login'}</button>
    </div>
  )
}
